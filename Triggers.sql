...
CREATE OR REPLACE FUNCTION fn_auditoria() RETURNS TRIGGER AS $fn_auditoria$
  ...
  -- Junto con la acción se escribe fecha y hora en la que se ha producido, y el usuario que provocó el trigger que la llamó
...
CREATE OR REPLACE TRIGGER tg_auditoria_disco after INSERT or UPDATE or DELETE
  ON disco 
  FOR EACH ROW
  EXECUTE PROCEDURE fn_auditoria(); 

  CREATE OR REPLACE TRIGGER tg_auditoria_desea after INSERT or UPDATE or DELETE
  ON desea FOR EACH ROW
  EXECUTE PROCEDURE fn_auditoria(); 

  CREATE OR REPLACE TRIGGER tg_auditoria_tiene after INSERT or UPDATE or DELETE
