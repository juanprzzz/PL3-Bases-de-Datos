...
Para quitar los usuarios anteriores hay que llevar a cabo los siguientes comandos, primero hay que quitar todos los permisos, y luego quitar los usuarios, una vez hecho eso, volver a ejecutar el .sql:
    Para admin:
    REVOKE ALL PRIVILEGES ON auditoria, cancion, desea, disco, edicion, genero, grupo, tiene, usuario FROM admin; DROP USER admin;
    Para gestor:
    REVOKE ALL PRIVILEGES ON auditoria, cancion, desea, disco, edicion, genero, grupo, tiene, usuario FROM gestor; DROP USER gestor;
    Para cliente:
    REVOKE ALL PRIVILEGES ON tiene, desea FROM cliente; DROP USER cliente;
    Para invitado:
    REVOKE ALL PRIVILEGES ON disco, cancion FROM invitado; DROP USER invitado;
*/
